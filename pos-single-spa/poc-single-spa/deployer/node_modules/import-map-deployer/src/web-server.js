...
app.use((req, res, next) => {
  res.header("Access-Control-Allow-Origin", "*"); // Разрешаем доступ для всех источников
  res.header("Access-Control-Allow-Methods", "GET, POST, PUT, DELETE, OPTIONS"); // Указываем разрешенные методы
  res.header("Access-Control-Allow-Headers", "Origin, X-Requested-With, Content-Type, Accept"); // Указываем разрешенные заголовки
  next();
})
...
app.patch("/import-map.json", (req, res) => {
  ...
  if (req.body.imports) {
    ...
    if (inputFormatIssues.length > 0) {
      console.log('hello2', inputFormatIssues.join("\n"))
