...
function* getGoodsListFilesSaga() {
  const params = paramsGenerator([['obf_category', ObfCategories.GOODS_LIST]]);
...
function* goodsListSaga({ payload }: IGetCatalogSagaState) {
  ...
  if (listStatus === NoIndexChecks.NOT_CHECKED && !noIndex) {
  ...
  if (response?.data?.data) {
    ...
    if (listStatus === NoIndexChecks.NOT_CHECKED && noIndex) {
      if (records) {
        yield put(setGoodsListStatus(NoIndexChecks.HAS_UNPUBLISHED));
      } else {
        yield all([put(setGoodsListStatus(NoIndexChecks.NO_UNPUBLISHED)), put(setGoodsListFilters({ noIndex: false }))]);
        return;
      }
    }
    //TODO: проверить
