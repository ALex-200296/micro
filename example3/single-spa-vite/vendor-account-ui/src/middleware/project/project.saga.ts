...
function* updateProjectDetailsSaga({ payload }: IUpdateRequestDetailsSagaState) {
  ...
  if (postResponse) {
    ...
    if (computedProperty === ProjectsComputedProperty.PROJECT_REQUESTS) {
      const actionId = `${projectSliceName}/${ProjectsComputedProperty.PROJECT_REQUESTS}`;
      yield all([
...
function* getProjectRequestsListSaga({ payload }: IGetProjectsRequestsSagaState) {
  ...
  if (response?.data) {
    yield all([
      put(setProjectsData({ list: data, computedProperty: ProjectsComputedProperty.PROJECT_REQUESTS })),
