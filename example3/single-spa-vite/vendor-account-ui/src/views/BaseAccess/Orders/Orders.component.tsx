...
const Orders: React.FC = () => {
  ...
  const filterStatusOptions = useSelector(
    infoSelectors.getInfoParams(InfoParamsOper.INVOICE_STATUS, undefined, InfoParamsGroup.ORDERS),
  );
  ...
  useEffect(() => {
    if (!filterStatusOptions.length) {
      dispatch(
        infoParamsAction({
          oper: InfoParamsOper.INVOICE_STATUS,
          group: InfoParamsGroup.ORDERS,
        }),
      );
    }
  ...
  const operationsMapper: OperationsMapperType = {
    [InvoiceOperations.MERCH_KDK]: onCrossDockingClick,
    [InvoiceOperations.MERCH_PRINT_LABEL]: onPrintLabel,
  };
