...
const Images: React.FC<ISubSectionProps> = ({ id }) => {
  ...
  const filtersCount = useSelector(catalogSelectors.getCatalogFiltersCount(CatalogComputedPropertyState.IMAGES_LOAD));
  ...
  const {
    ...
    dataSelector: catalogSelectors.getCatalogActionsData(CatalogComputedPropertyState.IMAGES_LOAD),
    setUpdate: () => setReportsUpdate(CatalogComputedPropertyState.IMAGES_LOAD),
    resetFilters: () => resetReportsFilters(CatalogComputedPropertyState.IMAGES_LOAD),
    sliceName: catalogSliceName,
    computedProperty: CatalogComputedPropertyState.IMAGES_LOAD,
    type: typeImagesLoad,
    templateProps: {
      handleClose: handleTemplatesClose,
  ...
  const { filterTagsData } = useFilterToTag({
    filterConfig,
    onClose: (key) => dispatch(resetReportsFilter({ computedProperty: CatalogComputedPropertyState.IMAGES_LOAD, key })),
    onCloseAll: () => dispatch(resetReportsFilters(CatalogComputedPropertyState.IMAGES_LOAD)),
  });
  ...
  return (
    <>
      ...
      <Table
        columns={columns}
        dataSelector={catalogSelectors.getCatalogTableData(CatalogComputedPropertyState.IMAGES_LOAD)}
        pagination={{
          current: page,
          pageSize: rows,
          total: records,
