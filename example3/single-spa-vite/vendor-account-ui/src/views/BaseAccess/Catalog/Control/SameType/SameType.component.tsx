...
const SameType: React.FC<ISubSectionProps> = ({ id }) => {
  ...
  const filtersCount = useSelector(catalogSelectors.getCatalogFiltersCount(CatalogComputedPropertyState.SAME_TYPE_LOAD));
  ...
  const {
    ...
    dataSelector: catalogSelectors.getCatalogActionsData(CatalogComputedPropertyState.SAME_TYPE_LOAD),
    setUpdate: () => setReportsUpdate(CatalogComputedPropertyState.SAME_TYPE_LOAD),
    resetFilters: () => resetReportsFilters(CatalogComputedPropertyState.SAME_TYPE_LOAD),
    sliceName: catalogSliceName,
    computedProperty: CatalogComputedPropertyState.SAME_TYPE_LOAD,
  ...
  const { filterTagsData } = useFilterToTag({
    filterConfig,
    onClose: (key) =>
      dispatch(resetReportsFilter({ computedProperty: CatalogComputedPropertyState.SAME_TYPE_LOAD, key })),
    onCloseAll: () => dispatch(resetReportsFilters(CatalogComputedPropertyState.SAME_TYPE_LOAD)),
  });
  ...
  return (
    <>
      ...
      <Table
        columns={columns}
        dataSelector={catalogSelectors.getCatalogTableData(CatalogComputedPropertyState.SAME_TYPE_LOAD)}
        pagination={{
          current: page,
          pageSize: rows,
          total: records,
