...
import { dataTestId, purchaseInfoData } from './Price.data';

const PriceComponent: React.FC = () => {
  const dispatch = useDispatch();

  const filtersCount = useSelector(pricingSelectors.getPricingFiltersCount(PricingComputedPropertyState.PRICE_LOAD));
  ...
  const {
    ...
    dataSelector: pricingSelectors.getPricingActionsData(PricingComputedPropertyState.PRICE_LOAD),
    setUpdate: () => setReportsUpdate(PricingComputedPropertyState.PRICE_LOAD),
    resetFilters: () => resetReportsFilters(PricingComputedPropertyState.PRICE_LOAD),
    sliceName: pricingSliceName,
    computedProperty: PricingComputedPropertyState.PRICE_LOAD,
    type: typePriceLoad,
    templateProps: {
      handleClose: handleTemplatesClose,
  ...
  const { fileLoadData, infoTitle, manualsData } = purchaseInfoData;
  ...
  const { filterTagsData } = useFilterToTag({
    filterConfig,
    onClose: (key) => dispatch(resetReportsFilter({ computedProperty: PricingComputedPropertyState.PRICE_LOAD, key })),
    onCloseAll: () => dispatch(resetReportsFilters(PricingComputedPropertyState.PRICE_LOAD)),
  });
  ...
  return (
    <>
      ...
      <Table
        columns={columns}
        dataSelector={pricingSelectors.getPricingTableData(PricingComputedPropertyState.PRICE_LOAD)}
        pagination={{
          current: page,
          pageSize: rows,
          total: records,
