...
const Services: React.FC = () => {
  ...
  const statusOptions = useSelector(
    infoSelectors.getInfoParams(InfoParamsOper.INVOICE_STATUS, undefined, InfoParamsGroup.SERVICES_ACTS),
  );
  ...
  const initialFiltersValues: IInvoiceFiltersInitialValues = {
    [InvoiceFilters.DATE_RANGE]: [startDate, endDate],
    [InvoiceFilters.STATUS]: status,
    [InvoiceFilters.ORDER_CODE]: orderCode,
  };
  ...
  const getServicesActs = useCallback(() => {
    dispatch(
      getInvoicesAction({
        ...
        group: InfoParamsGroup.SERVICES_ACTS,
  ...
  useEffect(() => {
    if (!statusOptions.length) {
      dispatch(infoParamsAction({ oper: InfoParamsOper.INVOICE_STATUS, group: InfoParamsGroup.SERVICES_ACTS }));
    }
