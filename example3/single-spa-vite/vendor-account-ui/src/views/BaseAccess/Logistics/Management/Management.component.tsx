...
const Management: React.FC = () => {
  ...
  const filtersCount = useSelector(
    logisticsSelectors.getLogisticsFiltersCount(LogisticsComputedPropertyState.STORAGE_ASSORTMENT),
  );
  ...
  const {
    ...
    dataSelector: logisticsSelectors.getLogisticsAdditionalData(LogisticsComputedPropertyState.STORAGE_ASSORTMENT),
    setUpdate: () => setReportsUpdate(LogisticsComputedPropertyState.STORAGE_ASSORTMENT),
    resetFilters: () => resetReportsFilters(LogisticsComputedPropertyState.STORAGE_ASSORTMENT),
    sliceName: logisticsSliceName,
    computedProperty: LogisticsComputedPropertyState.STORAGE_ASSORTMENT,
  ...
  const { filterTagsData } = useFilterToTag({
    filterConfig,
    onClose: (key) =>
      dispatch(resetReportsFilter({ computedProperty: LogisticsComputedPropertyState.STORAGE_ASSORTMENT, key })),
    onCloseAll: () => dispatch(resetReportsFilters(LogisticsComputedPropertyState.STORAGE_ASSORTMENT)),
  });
  ...
  return (
    <>
      ...
      <Table
        columns={columns}
        dataSelector={logisticsSelectors.getLogisticsTableData(LogisticsComputedPropertyState.STORAGE_ASSORTMENT)}
        pagination={{
          current: page,
          pageSize: rows,
          total: records,
