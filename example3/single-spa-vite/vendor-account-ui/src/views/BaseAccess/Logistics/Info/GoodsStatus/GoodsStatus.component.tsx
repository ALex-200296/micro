...
const GoodsStatus: React.FC<ISubSectionProps> = ({ id }) => {
  const dispatch = useDispatch();

  const filtersCount = useSelector(
    logisticsSelectors.getLogisticsFiltersCount(LogisticsComputedPropertyState.GOODS_STATUS),
  );
  ...
  const {
    ...
    dataSelector: logisticsSelectors.getLogisticsAdditionalData(LogisticsComputedPropertyState.GOODS_STATUS),
    setUpdate: () => setReportsUpdate(LogisticsComputedPropertyState.GOODS_STATUS),
    resetFilters: () => resetReportsFilters(LogisticsComputedPropertyState.GOODS_STATUS),
    sliceName: logisticsSliceName,
    computedProperty: LogisticsComputedPropertyState.GOODS_STATUS,
    type: typeGoodsStatus,
    templateProps: {
      handleClose: handleTemplatesClose,
  ...
  const { filterTagsData } = useFilterToTag({
    filterConfig,
    onClose: (key) =>
      dispatch(resetReportsFilter({ computedProperty: LogisticsComputedPropertyState.GOODS_STATUS, key })),
    onCloseAll: () => dispatch(resetReportsFilters(LogisticsComputedPropertyState.GOODS_STATUS)),
  });
  ...
  return (
    <>
      ...
      <Table
        columns={columns}
        dataSelector={logisticsSelectors.getLogisticsTableData(LogisticsComputedPropertyState.GOODS_STATUS)}
        pagination={{
          current: page,
          pageSize: rows,
          total: records,
...
export default memo(GoodsStatus);
