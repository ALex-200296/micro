...
export const getInvoiceBody = (
  ...
  switch (procedure) {
    case InvoiceProcedure[invoiceOperations.merchAccept]: {
    ...
    case InvoiceProcedure[invoiceOperations.merchReject]: {
      const lines = Object.entries(body as IInvoiceBodyParams[typeof procedure]).map(([key, value]) => ({
        linenum: key,
        extra2_MerchDate: dayjs(value.supplierDate).format(slashedFormat),
    ...
    case InvoiceProcedure[invoiceOperations.merch1C]:
      return formDataGenerator([['mail', email]]);
    case InvoiceProcedure[invoiceOperations.merchSplit]:
      return datalist
        ? formDataGenerator([
            [
              'gds',
        ...
        : null;
    case InvoiceProcedure[invoiceOperations.updateCDK]:
