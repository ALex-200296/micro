...
const Download: React.FC = () => {
  ...
  const {
    ...
    dataSelector: catalogSelectors.getCatalogActionsData(CatalogComputedPropertyState.newGoodsLoad),
    setUpdate: () => setReportsUpdate(CatalogComputedPropertyState.newGoodsLoad),
    resetFilters: () => resetReportsFilters(CatalogComputedPropertyState.newGoodsLoad),
    sliceName: catalogSliceName,
    computedProperty: CatalogComputedPropertyState.newGoodsLoad,
  ...
  const filtersCount = useSelector(catalogSelectors.getCatalogFiltersCount(CatalogComputedPropertyState.newGoodsLoad));
  ...
  const { filterTagsData } = useFilterToTag({
    filterConfig,
    onClose: (key) =>
      dispatch(resetReportsFilter({ computedProperty: CatalogComputedPropertyState.newGoodsLoad, key })),
    onCloseAll: () => dispatch(resetReportsFilters(CatalogComputedPropertyState.newGoodsLoad)),
  });
  ...
  const onFinish = useCallback((values: IFileLoadInitialValuesState) => {
    ...
    dispatch(
      postTaskAction({
        ...
        actions: [{ type: resetReportsData.type, payload: CatalogComputedPropertyState.newGoodsLoad }],
      }),
    );
  ...
  return (
    <>
      ...
      <Table
        columns={columns}
        dataSelector={catalogSelectors.getCatalogTableData(CatalogComputedPropertyState.newGoodsLoad)}
        pagination={{
          current: page,
          total: records,
          pageSize: rows,
