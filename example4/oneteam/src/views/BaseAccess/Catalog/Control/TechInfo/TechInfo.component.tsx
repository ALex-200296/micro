...
const TechInfo: React.FC<ISubSectionProps> = ({ id }) => {
  ...
  const filtersCount = useSelector(catalogSelectors.getCatalogFiltersCount(CatalogComputedPropertyState.techInfoLoad));
  ...
  const {
    ...
    dataSelector: catalogSelectors.getCatalogActionsData(CatalogComputedPropertyState.techInfoLoad),
    setUpdate: () => setReportsUpdate(CatalogComputedPropertyState.techInfoLoad),
    resetFilters: () => resetReportsFilters(CatalogComputedPropertyState.techInfoLoad),
    sliceName: catalogSliceName,
    computedProperty: CatalogComputedPropertyState.techInfoLoad,
  ...
  const { filterTagsData } = useFilterToTag({
    filterConfig,
    onClose: (key) =>
      dispatch(resetReportsFilter({ computedProperty: CatalogComputedPropertyState.techInfoLoad, key })),
    onCloseAll: () => dispatch(resetReportsFilters(CatalogComputedPropertyState.techInfoLoad)),
  });
  ...
  return (
    <>
      ...
      <Table
        columns={columns}
        dataSelector={catalogSelectors.getCatalogTableData(CatalogComputedPropertyState.techInfoLoad)}
        pagination={{
          current: page,
          pageSize: rows,
          total: records,
