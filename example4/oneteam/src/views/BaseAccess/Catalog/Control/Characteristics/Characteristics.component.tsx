...
const Characteristics: React.FC<ISubSectionProps> = ({ id }) => {
  ...
  const filtersCount = useSelector(catalogSelectors.getCatalogFiltersCount(CatalogComputedPropertyState.goodsLoad));
  const {
    filters: { date, status },
  } = useSelector(catalogSelectors.getCatalogActionsData(CatalogComputedPropertyState.goodsLoad));
  const {
    page: reportsPage,
    rows: reportsRows,
  ...
  const { filterTagsData } = useFilterToTag({
    filterConfig,
    onClose: (key) => dispatch(resetReportsFilter({ computedProperty: CatalogComputedPropertyState.goodsLoad, key })),
    onCloseAll: () => dispatch(resetReportsFilters(CatalogComputedPropertyState.goodsLoad)),
  });
  ...
  const onFinishCharacteristicsForm = useCallback((values: IFileLoadInitialValuesState) => {
    ...
    dispatch(
      postTaskAction({
        ...
        type: typeGoodsLoad,
        codeNotification,
        actions: [{ type: resetReportsData.type, payload: CatalogComputedPropertyState.goodsLoad }],
      }),
    );
