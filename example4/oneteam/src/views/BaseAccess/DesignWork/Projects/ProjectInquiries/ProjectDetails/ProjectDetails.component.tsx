...
const ProjectDetails: React.FC<ProjectDetailsProps> = ({ afterSubmit }) => {
  const id = useSelector(projectSelectors.getSelectedProjectId(ProjectsComputedProperty.projectRequests));
  const project = useSelector(projectSelectors.getProjectDetailsById(ProjectsComputedProperty.projectRequests));
  ...
  useEffect(() => {
    if (!project && id) {
      dispatch(getProjectRequestDetailsAction({ id, detailsType: ProjectsComputedProperty.projectRequests }));
    }
  }, [project, id]);
