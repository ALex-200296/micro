...
const ProjectInquiries: React.FC = () => {
  ...
  const { records, page, rows } = useSelector(
    projectSelectors.getProjectsPaginationData(ProjectsComputedProperty.projectRequests),
  );
  const isLoading = useSelector(
    uiSelectors.getIsRequestPending(`${projectSliceName}/${ProjectsComputedProperty.projectRequests}`),
  );
  const extCode = useSelector(projectSelectors.getExtCodeOfSelectedProject(ProjectsComputedProperty.projectRequests));

  const onTableChange = useOnTableChange({
    onPaginate: setProjectsPagination,
    computedProperty: ProjectsComputedProperty.projectRequests,
  });
    ...
    <>
      ...
      <Table
        ...
        onRow={(record) => ({
          onClick: () => {
            dispatch(
              setSelectedProjectId({ id: record.id, computedProperty: ProjectsComputedProperty.projectRequests }),
            );
            handleDetailsOpen();
          },
        })}
