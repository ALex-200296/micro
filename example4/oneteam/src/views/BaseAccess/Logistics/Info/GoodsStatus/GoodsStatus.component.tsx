...
const GoodsStatus: React.FC<ISubSectionProps> = ({ id }) => {
  ...
  const filtersCount = useSelector(
    logisticsSelectors.getLogisticsFiltersCount(LogisticsComputedPropertyState.goodsStatus),
  );
  ...
  const {
    ...
    dataSelector: logisticsSelectors.getLogisticsAdditionalData(LogisticsComputedPropertyState.goodsStatus),
    setUpdate: () => setReportsUpdate(LogisticsComputedPropertyState.goodsStatus),
    resetFilters: () => resetReportsFilters(LogisticsComputedPropertyState.goodsStatus),
    sliceName: logisticsSliceName,
    computedProperty: LogisticsComputedPropertyState.goodsStatus,
  ...
  const { filterTagsData } = useFilterToTag({
    filterConfig,
    onClose: (key) =>
      dispatch(resetReportsFilter({ computedProperty: LogisticsComputedPropertyState.goodsStatus, key })),
    onCloseAll: () => dispatch(resetReportsFilters(LogisticsComputedPropertyState.goodsStatus)),
  });
  ...
  return (
    <>
      ...
      <Table
        columns={columns}
        dataSelector={logisticsSelectors.getLogisticsTableData(LogisticsComputedPropertyState.goodsStatus)}
        pagination={{
          current: page,
          pageSize: rows,
          total: records,
