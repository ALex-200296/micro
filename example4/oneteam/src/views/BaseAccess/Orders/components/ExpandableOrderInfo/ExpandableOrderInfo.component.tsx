...
const ExpandableOrderInfo: React.FC<IExpandableOrderInfoProps> = ({
  ...
  const operation = useMemo(
    () =>
    {
      if (tableIsEditable[id]) {
        const entry = Object.entries(tableIsEditable[id]).find(([, value]) => !!value);
        return entry ? (entry[0] as keyof typeof invoiceOperations) : undefined;
      }
      return undefined
    },
  ...
  const columns = useMemo(
    () =>
      getOrdersColumnsConfig(
        statusCode,
        statusRejectOptions,
        getSortingForTable(operation ? 'asc' : extendedSorts[ExtendedSort.requiredDeliveryDate]),
        operation,
        setCheckedRows,
